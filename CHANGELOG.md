...
- Added a rate limit to the shelf rescan (`POST /api/shelves/{id}/scans`) of five walks back to back plus one every 10 seconds, refusing the rest with `429` and a `Retry-After` so a LAN device cannot hold the server's CPU and SMB bandwidth with a loop; `409` still means another walk is running, and it is not configurable.
