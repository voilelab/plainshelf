...
- Changed `book.json` writes to keep fields PlainShelf does not recognize. A key you added to a book's `book.json` by hand — `series`, `douban_id`, a note of your own — used to disappear the next time PlainShelf wrote that book, which a rating, a cover upload, a move to another layer, and a source switch all do, because the file was read into a fixed set of fields and rewritten whole. Such keys are now written back unchanged, values included: nested objects, array order, escapes, and the exact digits of a number all survive, because nothing about them is decoded. A value holding `&` or `<` also keeps those characters instead of being rewritten into an escape sequence. What is not preserved is where a key sits — the fields PlainShelf knows are written first, in its own order, and everything else follows in alphabetical order — and a key PlainShelf knows always wins a name clash, so the file never ends up with two values for one key. A book with no hand-added fields is written exactly as before. The kept keys stay in the file: they are not returned by the HTTP API and not copied into the exported book cache, because nothing in PlainShelf can interpret them. The same guarantee does not extend to `sources/{id}/meta.json` or `trash.json`, which are still read into their known fields and rewritten whole.
