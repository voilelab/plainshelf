...
- Added `plainshelf-read`, a standalone reading binary. Pointed at a shelf folder it opens the library in the browser and serves it for reading: no configuration file, no data directory, and no route that writes. The folder is opened read-only and left exactly as it was found, including the lock file, the `app/tmp/` directory and the exported book cache an ordinary server maintains, and the settings store it needs is kept in memory rather than written beside the binary. That makes it usable against storage a full server cannot open at all — a read-only mount, a filesystem snapshot, a backup copy. It binds `127.0.0.1` on a port the operating system picks and prints the address, so several copies can read several shelves at once with nothing to configure; `-no-browser` prints the address instead of opening one. Reading progress, bookmarks and reading history still work, kept by the browser per device as they already were. The interface is the one the server serves, minus what a reading client cannot use: editing, importing, the trash, the duplicate scan, the maintenance views and the server logs are neither served nor offered. Both binaries now ship in the release archive; see [Standalone Reader](docs/standalone-reader.md).

### Changed

- Changed `GET /api/mode` to report a `mode` field alongside `read_only`, naming which HTTP surface the server mounts: `full` for an ordinary server or the desktop app, `reader` for the standalone reading binary. The two answer different questions — a read-only server still serves every route and still administers itself, while a reader serves no trash, log, setting or task route at all — and the web client uses the new field to keep the pages it cannot use out of reach rather than let them fail. The matching `mode` config key accepts the same names; an unrecognized one now fails startup instead of quietly serving the whole API. `read_only` is unchanged, and a server that sets no mode behaves exactly as before.
