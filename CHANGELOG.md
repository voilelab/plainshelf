...
- Added a `show_nsfw` server setting (`GET`/`POST`/`DELETE /api/setting/show_nsfw`, off by default) that hides every book the shelf marks as adult content across the whole API — the library listing, the duplicate and similarity results, the dashboard's totals, the folder tree, the trash listing, the fingerprint coverage count, and the batch and background sweeps — and answers `404` rather than `403` for a book, cover, content or source named directly, or for a trashed book named for restore or permanent deletion; the exported book cache stays a complete mirror whatever it is set to, and emptying the trash still erases every book in it (a filtered trash listing carries `X-PlainShelf-Trash-Partial: true`, so the confirmation asks about the whole trash instead of naming a count it would understate).
- Added a **Show adult content** switch under **Settings → Adult content** for `show_nsfw`, which refetches the library listing and the folder tree on change so both agree with the setting without a reload.
