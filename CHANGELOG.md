...
- Added an adult-content mark that travels with the shelf: `content.nsfw_folders` in `shelf.json` marks a folder and every folder below it (a book's own `"nsfw": false` cannot cancel that), `"nsfw": true` in a `book.json` marks one book, and each entry of the exported book cache carries the assembled answer; nothing is hidden by it yet.
