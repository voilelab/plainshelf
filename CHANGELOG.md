...
- Fixed **Update now** for the mobile book cache stopping at the first read-only shelf and skipping every shelf after it.
- Fixed the duplicate-content page offering its fingerprint sweep on a read-only shelf, where the sweep is refused with `409`.
