...
- Added a character-count range filter to the book list. The library toolbar now carries a minimum and a maximum character count; leaving both empty keeps the list unfiltered, and setting either one narrows it. The bounds are held in the URL as `minChars` and `maxChars`, so a filtered view can be bookmarked and shared, and they combine with the folder, search, and sort controls already on the page. Bounds entered in reverse order are stored in order. A book whose character count cannot be read still counts as zero, and while the filter is on the toolbar reports how many of the listed books those are and offers the content-statistics sweep that computes them. Character counts are fetched only while a range is set. The filter is not offered on the Android client, whose pCloud mode would have to read every book's source metadata over the network to answer the same request.
- Added case-sensitive, whole-word, and regular-expression matching to the source editor's find and replace, alongside a highlight over every match in the document and a "match x of N" count. In a regular-expression replacement, `$1`…`$9` insert the corresponding capture group and `$&` the whole match. An expression that cannot be parsed is reported in the status line instead of being searched with. The "current chapter" scope now filters matches rather than searching a copy of the chapter, so finding, replacing, replace-all, and the highlight all agree on the same range, and the range follows the chapter's text as the document is edited.
- Added a working **Export file** action to the Android client's book detail page. The button had done nothing there: the web build falls back to a blob `<a download>` click that the Android WebView silently ignores, so the tap produced neither a file nor an error. The mobile client now writes the book's text through the Filesystem plugin into a shared `Documents/PlainShelf/` folder and confirms the saved location with a toast. On Android 11 and above the Filesystem plugin reaches the shared Documents folder with no permission; on Android 10 and below it needs both `READ_EXTERNAL_STORAGE` and `WRITE_EXTERNAL_STORAGE`, declared capped at API 29. A browser preview keeps the existing blob-download fallback.

### Changed

- **Breaking (pre-1.0):** renamed the "layer" concept to "folder" across every outward surface, so the API, the URL, and the UI all use one word for a book's place in the tree. The folder-tree routes are now `GET /folders`, `POST /folder-moves`, `POST /folder-transfers`, and `POST`/`PATCH`/`DELETE /folders/{path}`; the old `/layers`, `/layer-moves`, and `/layer-transfers` paths are gone and answer `404`, with no compatibility alias. The book JSON key is now `folder` — the transitional `layers` field and its dual read are removed — and the move, transfer, batch, scan, and trash payloads use `folder`, `target_folder`, `source_folder`, `folder_count`, and `original_folder`. The library filters on `?folders=` instead of `?layers=`, so an existing `?layers=` bookmark no longer selects a folder. Two on-disk caches move with it and need no migration step: `trash.json` renames `original_layer` to `original_folder` and bumps its schema to v2, and each exported `app/book-cache-*.json` renames `layers` to `folders` and bumps its schema to v2 — an older build now refuses to modify a v2 trash record rather than rewriting it and dropping the restore path, and a stale v1 book cache is treated as a plain cache miss and rebuilt with no error surfaced. A book trashed by a pre-upgrade build still restores, but to the top of `books/` rather than its original folder, because the old `original_layer` value is no longer read. The `books/` directory layout, `book.json`, and the book and source schema versions are unchanged: a shelf created before this opens and lists every book and folder with no migration.
- Changed a full shelf scan so it no longer lists every folder under `books/` to find out that nothing moved. A directory's modification time changes whenever one of its direct children is added, removed, or renamed, so each scan now records those times alongside the entries it found and the next scan replaces the directory listing with a single stat wherever that time — and the time of the folder above it — is unchanged. On a fixture of 3061 folders the second scan issues no listings at all and takes about a third of the time; on a network mount the saving is larger, because there a listing is several round trips and a stat is one. The record is kept in `app/scan-cache.json` and loaded before the first scan of a process, so the scan a user waits for at startup is already cheap; it is written after that first scan and again at shutdown, an unchanged record is not rewritten, and pressing **Update book list** still writes nothing. Anything missing, unreadable, or written by a newer build is treated as a cache miss and the shelf is walked in full. What a book contains is unaffected: a directory's time says nothing about its subdirectories, so `book.json` is still checked by the per-book freshness check exactly as before, and a folder modified within the last two seconds is left out of the record entirely so that a coarse filesystem clock cannot hide a change made moments after a scan. The new `scan_cache: off` setting disables it for a mount whose directory times cannot be trusted — some cloud storage gateways do not update them when a child is added.
- Changed `GET /books?include=char_count` to answer from the shelf's in-memory book cache. It used to open every book's current source and decode its `meta.json` on each request, on top of a stat of the source folder — two filesystem operations per book, which is the cost the book cache exists to keep off a listing and which the home page paid on every load, most noticeably on an SMB or other network-mounted shelf. Each book's character count is now held beside it in the cache and filled whenever that entry is built, so asking for counts adds no disk operation to a listing. They stay correct as they change: every route that rewrites a source's content, recomputes its statistics, or moves the current-source pointer refreshes the cached count before it answers. A source edited outside PlainShelf is the exception and behaves like every other external change — its new count appears at the next full scan, or immediately after **Update book list**. In exchange, a full scan now also reads each book's current source `meta.json`, one extra operation per book. The response shape is unchanged.
- Changed the hint file each book folder carries. It is now named `CURRENT_SOURCE.txt`, matching the "source" wording the rest of the data model uses, and its content is written in English — it used to be `CURRENT_VERSION_LOCATION.txt` with hardcoded Traditional Chinese text, which an English-speaking reader opening a `.bookpkg` folder could not read. The server has no locale setting to follow, so English is the fixed choice rather than one that tracks the UI language. The text now also says what the file is: a hint that nothing reads back, safe to delete, with `current_source` in `book.json` remaining the source of truth. Existing shelves need no migration — the next time a book's current source changes, the new file is written and the old one is removed, so a book never carries both.
- Changed the source editor to CodeMirror 6. It used to be a plain text box that could only hold one chapter of the source at a time, which meant every position had to be converted across a boundary that moved while the text was edited, and scrolling to a position was guessed by laying the text out a second time off-screen. Both were sources of the editor landing somewhere other than where it was asked to — after a search, after a chapter jump, or after editing near a chapter's edge — and neither could be fixed by adjusting the arithmetic. The editor now holds the whole source at once and lets CodeMirror carry positions through edits, so find results, chapter jumps, and the caret land where they should. Focusing a chapter from the outline is now only a visual change: the rest of the source is dimmed rather than removed, so switching chapters no longer discards the undo history, and typing across a chapter boundary no longer shifts the view. A source written with CRLF line endings is still saved with them. Markdown sources are now syntax-highlighted — headings, emphasis, quotes, lists, inline code, and the HTML tags the reader renders — while a plain-text source is left as prose, and pressing Enter inside a list or a quote continues it. The editor page's JavaScript grows from 11 kB to 184 kB compressed; it is loaded only when that page is opened, so every other page is unaffected.
- Changed how a new book's ID is generated. It used to be the first 8 hex characters of a hash of the book's layer path and title, which is narrow enough that a shelf of 10,000 books had a 1.2% chance of two books sharing one, and the check meant to catch that could only see books the server already knew about — so a book added from another machine sharing the shelf, or copied in with a file manager, could end up with an ID a book already had, and the API and the Android client would then treat the two as one book. New books get a random version 4 UUID instead, wide enough that a collision is not something a shelf can realistically produce. Existing books keep their IDs exactly as they are: nothing is migrated or recomputed, the older forms work side by side with the UUIDs in the same shelf, and reading progress, bookmarks, and Android downloads are unaffected.
- Changed the Android client to require a book to be downloaded before it can be read. It used to stream a not-yet-downloaded book's content on demand from the server or the pCloud backend, so reading worked with no offline copy at all. The mobile router guard now refuses the reader route for a book that is not downloaded and redirects to the book's detail page, which prompts the user to download first; a book already downloaded, including one with a pending update, opens as before, and a failure to read the download state is treated as not-downloaded so the requirement cannot leak through. The download itself now writes its `manifest.json` last, after the content, sources, and cover, so a book counts as downloaded only once its text is actually on disk — an interrupted download leaves manifest-less orphan files the cache already ignores rather than a book listed as downloaded without its content.
