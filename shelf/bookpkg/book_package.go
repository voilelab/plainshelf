...
// ErrNotBookPackage reports a directory that opened but holds no book.json. It
// is distinct from a decode failure: a missing book.json means "wrong folder",
// a malformed one means "this book is damaged".
...
//
// It is opened read-only, so every mutation on the returned Book is refused
// before it touches a directory the caller may not own. Most report
// fsutil.ErrReadOnly, but a book.json newer than this build is refused earlier
// as ErrUnsupportedBookSchemaVersion.
type BookPackage struct {
	root *os.Root
	book *Book
}

// OpenBookPackage opens the book package in dir for reading. A nil logger
// discards the package's own warnings.
//
...
func OpenBookPackage(dir string, logger *logutil.Logger) (*BookPackage, error) {
	...
	// The package is its own root, so nothing outside the folder the user chose
	// is reachable through this handle.
