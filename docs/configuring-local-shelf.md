...
While such a shelf is the active one, the web and desktop UI says so in a banner and leaves out the actions it would refuse — importing, editing metadata, sources and covers, creating and moving folders, deleting, and restoring from the trash. Browsing, reading, searching, and **Rescan shelf** stay available, and so does copying a book or folder *out* to another shelf, since only the target is written; the same transfer's **Move** mode is not offered, because it would have to delete the source. A read-only shelf is also left out of the transfer destinations other shelves offer. Every other shelf on the same server keeps its own actions, and the refusal itself is unchanged: a request sent anyway is still refused by the server.
