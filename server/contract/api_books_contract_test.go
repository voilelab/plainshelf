...
// A patch distinguishes an absent field from one carrying a value, and both v1
// and v2 read an explicit null the same way they read an absent field: the
// pointer stays nil and the field is left alone. Clearing a list is therefore
// spelled [] and never null, and the API's move to json/v2 does not move that
// line.
func TestAPIUpdateBookPointerFieldsKeepTheirPatchSemanticsContract(t *testing.T) {
	env := newAPITestEnv(t)
	created := importTextBook(t, env, "Pointer Fields", "", "pointer.txt", "body")
	bookID := created.Meta.ID

	seeded := patchBookOK(t, env, bookID,
		`{"authors":["Author A"],"tags":["tag1"],"identifiers":{"isbn":"1"}}`)
	if len(seeded.Meta.Authors) != 1 || len(seeded.Meta.Tags) != 1 || len(seeded.Meta.Identifiers) != 1 {
		t.Fatalf("seeded meta = %+v, want one author, tag and identifier", seeded.Meta)
	}

	// An omitted field leaves the stored value alone.
	untouched := patchBookOK(t, env, bookID, `{"title":"Pointer Fields"}`)
	if len(untouched.Meta.Authors) != 1 || len(untouched.Meta.Tags) != 1 || len(untouched.Meta.Identifiers) != 1 {
		t.Errorf("after an omitted patch meta = %+v, want the seeded values kept", untouched.Meta)
	}

	// An explicit null does the same: it is not a way to clear a field.
	nulled := patchBookOK(t, env, bookID, `{"authors":null,"tags":null,"identifiers":null}`)
	if len(nulled.Meta.Authors) != 1 || len(nulled.Meta.Tags) != 1 || len(nulled.Meta.Identifiers) != 1 {
		t.Errorf("after a null patch meta = %+v, want the seeded values kept", nulled.Meta)
	}

	// An empty list is what clears a field.
	cleared := patchBookOK(t, env, bookID, `{"authors":[],"tags":[],"identifiers":{}}`)
	if len(cleared.Meta.Authors) != 0 || len(cleared.Meta.Tags) != 0 || len(cleared.Meta.Identifiers) != 0 {
		t.Errorf("after an empty-list patch meta = %+v, want the three fields cleared", cleared.Meta)
	}

	// The cleared authors list comes back as [], not null - the response side of
	// the same contract.
	rec := patchBook(t, env, bookID, `{"authors":[]}`, http.StatusOK)
	assertJSONArray(t, rec.Body.Bytes(), "meta", "authors")
}

// omitempty still wins over the new "[] rather than null" default: v2 defines it
// as "would encode as null, empty string, empty object or empty array", so an
// empty tags list stays absent instead of turning into []. Which fields carry it
// is therefore what decides whether a client can drop its own null guard, and
// for tags and identifiers the answer is still no.
func TestAPIOmitEmptyMetaFieldsStayOmittedContract(t *testing.T) {
	env := newAPITestEnv(t)
	created := importTextBook(t, env, "Omitted", "", "omitted.txt", "body")

	rec := patchBook(t, env, created.Meta.ID, `{"format":"txt"}`, http.StatusOK)
	body := decodeJSON[map[string]any](t, rec)

	meta, ok := body["meta"].(map[string]any)
	if !ok {
		t.Fatalf("meta = %#v, want an object", body["meta"])
	}
	// tags and identifiers are omitempty on BookMeta, so an empty one is absent
	// rather than [] - unlike authors, which carries no omitempty.
	for _, field := range []string{"tags", "identifiers"} {
		if _, present := meta[field]; present {
			t.Errorf("%s = %#v, want the field omitted while empty", field, meta[field])
		}
	}
	if _, present := meta["authors"]; !present {
		t.Errorf("authors is absent; it carries no omitempty and must always be present")
	}
}
