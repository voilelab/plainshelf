...
import { importBookAs, helloMarkdownFixturePath } from './support/books';
...
test('should switch and persist the reading font without changing code text', async ({ page }) => {
  ...
  await importBookAs(page, helloMarkdownFixturePath, 'reader-fonts-hello');
  await page.locator('.book-list-row').getByRole('heading', { name: 'reader-fonts-hello', exact: true }).click();
