...
- **Updates.** [Dependabot](.github/dependabot.yml) opens grouped pull requests for the three Go modules, both npm lockfiles, the Dockerfile base images, the documentation build requirements, and the GitHub Actions workflows. What ships to users runs weekly — the Go modules, the `frontend` lockfile and the workflows; what does not runs monthly — the `e2e` lockfile, the base images and the documentation requirements. Cadence is a review-noise decision, not a security one: an actual advisory arrives through Dependabot security updates, which ignore this schedule entirely.
