...
function sniffImageMime(bytes: Uint8Array): string {
  ...
  // The full 8-byte PNG signature, not just \x89PNG: the trailing CRLF/EOF
  // bytes are what the format uses to detect mangled transfers, and checking
  // half a signature is how a near-miss gets labelled as an image type it
  // is not.
  if (
    bytes.length >= 8 &&
    bytes[0] === 0x89 &&
  ...
  // GIF87a or GIF89a, both six bytes; 'GIF' alone would match anything that
  // happens to start with those three characters.
  if (
    bytes.length >= 6 &&
    bytes[0] === 0x47 &&
    bytes[1] === 0x49 &&
